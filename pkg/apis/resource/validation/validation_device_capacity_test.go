...
func TestValidateDeviceCapacity(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"invalid-options-duplicate-normalized": {
			capacity: testDeviceCapacity(maxCapacity, testCapacitySharingPolicy(&oneKIAbbreviated, []apiresource.Quantity{oneKIAbbreviated, oneKIUnabbreviated}, nil)),
			wantFailures: field.ErrorList{
				field.Duplicate(validValuesField.Index(1), "1024"),
			},
		},
		"invalid-options-unsort": {
			capacity: testDeviceCapacity(maxCapacity, testCapacitySharingPolicy(&one, []apiresource.Quantity{two, one}, nil)),
			wantFailures: field.ErrorList{
