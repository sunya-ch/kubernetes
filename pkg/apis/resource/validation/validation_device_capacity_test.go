...
func TestValidateDeviceCapacity(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"policy-with-valid-range-without-default": {
			capacity: testDeviceCapacity(one, testCapacityRequestPolicy(nil, nil, testValidRange(ptr.To(one), nil, nil), nil)),
			wantFailures: field.ErrorList{
				field.Required(policyField.Child("default"), "required when validRange is defined"),
			},
		},
		"policy-with-valid-range-without-min": {
			capacity: testDeviceCapacity(one, testCapacityRequestPolicy(&one, nil, testValidRange(nil, nil, nil), nil)),
			wantFailures: field.ErrorList{
				field.Required(validRangeField.Child("min"), "required when validRange is defined"),
			},
		},
		"valid-simple-range": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), nil, nil), nil)),
		},
		"valid-range-with-max": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), ptr.To(maxCapacity), nil), nil)),
		},
		"valid-range-with-step": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), nil, ptr.To(one)), nil)),
		},
		"valid-range-with-max-and-step": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), ptr.To(maxCapacity), ptr.To(one)), nil)),
		},
		"valid-single-option": {
		...
		"more-than-one-policy": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one,
				[]apiresource.Quantity{one}, testValidRange(ptr.To(one), nil, nil), nil)),
			wantFailures: field.ErrorList{
		...
		"invalid-range-large-min-small-max": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&two, nil, testValidRange(ptr.To(overCapacity), ptr.To(one), nil), nil)),
			wantFailures: field.ErrorList{
		...
		"invalid-range-large-max": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), ptr.To(overCapacity), nil), nil)),
			wantFailures: field.ErrorList{
				field.Invalid(validRangeField.Child("max"), "20Gi", "max is larger than capacity value: 10Gi"),
			},
		},
		"invalid-range-multiple-of-step": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&two, nil, testValidRange(ptr.To(one), ptr.To(maxCapacity), ptr.To(two)), nil)),
			wantFailures: field.ErrorList{
		...
		"invalid-range-large-step": {
			capacity: testDeviceCapacity(maxCapacity, testCapacityRequestPolicy(&one, nil, testValidRange(ptr.To(one), nil, ptr.To(maxCapacity)), nil)),
			wantFailures: field.ErrorList{
