...
func numEntriesDeviceAttribute(device resource.Device) int {
	...
	for _, attr := range device.Attributes {
		...
		numEntries += len(attr.BoolValues) + len(attr.IntValues) + len(attr.StringValues) + len(attr.VersionValues)
	}
	return numEntries
}
	...
	for key, cap := range capacity {
		...
		if oldDevice != nil {
			if oldCap, ok := oldDevice.Capacity[key]; ok {
				oldPolicy = oldCap.RequestPolicy
			}
		}
...
func validateRequestPolicyRange(defaultValue apiresource.Quantity, maxCapacity apiresource.Quantity,
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.DRAFractionalCapacityRange) &&
		hasFractional && (oldRange == nil || !rangeHasFractional(*oldRange)) {
	...
	if valueRange.Step != nil {
		if valueRange.Step.Sign() <= 0 {
			...
			// correct alignment checks regardless of the gate state.
