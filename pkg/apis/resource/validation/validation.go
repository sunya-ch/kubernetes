...
func validateResourceClaimStatusUpdate(status, oldStatus *resource.ResourceClaimStatus, claimDeleted bool, requestNames requestNames, fldPath *field.Path) field.ErrorList {
	...
	validateItem := func(device resource.AllocatedDeviceStatus, fldPath *field.Path) field.ErrorList {
		return validateDeviceStatus(device, fldPath, allocatedDevices)
	}
	if utilfeature.DefaultFeatureGate.Enabled(features.DRAConsumableCapacity) || shareIDInUse(status) {
		allErrs = append(allErrs, validateSet(status.Devices, -1,
			...
			func(device resource.AllocatedDeviceStatus) (structured.SharedDeviceID, string) {
				...
				return structured.MakeSharedDeviceID(deviceID, (*types.UID)(device.ShareID)), "sharedDeviceID"
			},
...
func shareIDInUse(status *resource.ResourceClaimStatus) bool {
	if status == nil {
		return false
	}
	if devices := status.Devices; devices != nil {
