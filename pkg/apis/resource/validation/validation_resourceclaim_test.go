...
func TestValidateClaimStatusUpdate(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"invalid-add-allocated-status-invalid-share-id": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("status", "devices").Index(0).Child("shareID"), badShareIDStr, "unable to parse uid: invalid UUID length: 1"),
				field.Invalid(field.NewPath("status", "allocation", "devices", "results").Index(0).Child("shareID"), badShareIDStr, "unable to parse uid: invalid UUID length: 1"),
			},
			oldClaim: testClaim(goodName, goodNS, validClaimSpec),
			update: func(claim *resource.ResourceClaim) *resource.ResourceClaim {
				claim.Status.Allocation = &resource.AllocationResult{
