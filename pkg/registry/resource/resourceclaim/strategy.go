...
// dropDisabledDRAResourceClaimConsumableCapacityFields drops any new CapacityRequests, and ShareID
// fields from the newClaim if they were not used in the oldClaim.
func dropDisabledDRAResourceClaimConsumableCapacityFields(newClaim, oldClaim *resource.ResourceClaim) {
	...
	if allocation := newClaim.Status.Allocation; allocation != nil {
		for i := range allocation.Devices.Results {
			...
			newClaim.Status.Allocation.Devices.Results[i].ConsumedCapacities = nil
		}
	}
