...
var testCapacity = map[resource.QualifiedName]k8sresource.Quantity{
	resource.QualifiedName("test-capacity"): k8sresource.MustParse("1"),
}
...
func addSpecDeviceRequestWithCapacityRequests(resourceClaim *resource.ResourceClaim, request, class string, capacity map[resource.QualifiedName]k8sresource.Quantity) {
...
func addStatusAllocationDevicesResults(resourceClaim *resource.ResourceClaim, driver string, pool string, device string, request string,
	shareID *string, consumedCapacities map[resource.QualifiedName]k8sresource.Quantity) {
