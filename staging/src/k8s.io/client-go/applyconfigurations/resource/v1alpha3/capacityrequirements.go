...
type CapacityRequirementsApplyConfiguration struct {
	Requests map[resourcev1alpha3.QualifiedName]resource.Quantity `json:"requests,omitempty"`
}
...
func (b *CapacityRequirementsApplyConfiguration) WithRequests(entries map[resourcev1alpha3.QualifiedName]resource.Quantity) *CapacityRequirementsApplyConfiguration {
	if b.Requests == nil && len(entries) > 0 {
		b.Requests = make(map[resourcev1alpha3.QualifiedName]resource.Quantity, len(entries))
	}
