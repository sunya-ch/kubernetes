...
func (b *DeviceRequestAllocationResultApplyConfiguration) WithConsumedCapacities(entries map[resourcev1beta2.QualifiedName]resource.Quantity) *DeviceRequestAllocationResultApplyConfiguration {
	if b.ConsumedCapacities == nil && len(entries) > 0 {
		b.ConsumedCapacities = make(map[resourcev1beta2.QualifiedName]resource.Quantity, len(entries))
	}
