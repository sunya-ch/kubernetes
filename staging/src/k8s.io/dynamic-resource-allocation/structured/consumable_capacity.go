...
func calculateConsumedCapacity(requestedVal *resource.Quantity, consumable resourceapi.CapacitySharingPolicy) *resource.Quantity {
	if consumable.ValueRange != nil {
		if requestedVal == nil {
			returnedVal := consumable.Default.DeepCopy()
			return &returnedVal
		}
		if requestedVal.Cmp(consumable.ValueRange.Minimum) < 0 {
			returnedVal := consumable.ValueRange.Minimum.DeepCopy()
			return &returnedVal
		}
		if consumable.ValueRange.ChunkSize != nil {
			requestedInt64 := requestedVal.Value()
			chunksize := consumable.ValueRange.ChunkSize.Value()
			min := consumable.ValueRange.Minimum.Value()
	...
	} else if consumable.DiscreteValues != nil {
		if requestedVal == nil {
			returnedVal := consumable.Default.DeepCopy()
			return &returnedVal
		}
...
func violatePolicy(requestedVal resource.Quantity, policy *resourceapi.CapacitySharingPolicy) bool {
	...
	if policy.ValueRange != nil {
		if policy.ValueRange.Maximum != nil &&
			requestedVal.Cmp(*policy.ValueRange.Maximum) > 0 {
			return true
		}
		if policy.ValueRange.ChunkSize != nil {
			requestedInt64 := requestedVal.Value()
			chunksize := policy.ValueRange.ChunkSize.Value()
			min := policy.ValueRange.Minimum.Value()
			added := (requestedInt64 - min)
			mod := added % chunksize
			if mod != 0 {
				return true
	...
	if policy.DiscreteValues != nil {
		for _, validVal := range policy.DiscreteValues.Options {
