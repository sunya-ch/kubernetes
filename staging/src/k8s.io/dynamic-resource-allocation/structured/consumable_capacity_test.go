	...
	testcases := map[string]struct {
		...
		"less than maximum": {
			...
			&resourceapi.CapacitySharingPolicy{
				Default:    one,
				ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one, Maximum: &two},
			},
			false,
		},
		"more than maximum": {
			...
			&resourceapi.CapacitySharingPolicy{
				Default:    one,
				ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one, Maximum: &one},
			},
			true,
		},
		"in set": {
			...
			&resourceapi.CapacitySharingPolicy{
				Default:        one,
				DiscreteValues: &resourceapi.CapacitySharingPolicyDiscrete{Options: []resource.Quantity{one}},
			},
			false,
		},
		"not in set": {
...
func TestCalculateConsumedCapacity(t *testing.T) {
	testcases := map[string]struct {
		...
		"min in range": {
			nil,
			resourceapi.CapacitySharingPolicy{Default: one, ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one}},
			&one,
		},
		"default in set": {
			nil,
			resourceapi.CapacitySharingPolicy{Default: one, DiscreteValues: &resourceapi.CapacitySharingPolicyDiscrete{Options: []resource.Quantity{one}}},
			&one,
		},
		...
		"less than min in range": {
			&one,
			resourceapi.CapacitySharingPolicy{Default: one, ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: two}},
			&two,
		},
		"with step (round up)": {
			&two,
			resourceapi.CapacitySharingPolicy{Default: one, ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one, ChunkSize: ptr.To(two.DeepCopy())}},
			&three,
		},
		"with step (no remaining)": {
			&two,
			resourceapi.CapacitySharingPolicy{Default: one, ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one, ChunkSize: ptr.To(one.DeepCopy())}},
			&two,
		},
	}
	for name, tc := range testcases {
	...
	consumableCapacity := map[resourceapi.QualifiedName]resourceapi.DeviceCapacity{
		capacity0: {
			...
			SharingPolicy: &resourceapi.CapacitySharingPolicy{
				Default:    one,
				ValueRange: &resourceapi.CapacitySharingPolicyRange{Minimum: one},
			},
		},
		capacity1: {
			Value: two,
