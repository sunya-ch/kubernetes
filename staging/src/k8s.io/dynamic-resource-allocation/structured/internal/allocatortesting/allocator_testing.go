...
func TestAllocator(t *testing.T,
	...
	testcases := map[string]struct {
		...
		"consumable-capacity-multi-allocatable-device-allocation-mode-all-in-use-insufficient": {
			...
			node: node(node1, region1),
			// Expecting result if considering capacity check is one of selector
			// Expecting nil if capacity check is not considered as allocated status
			expectResults: []any{
				allocationResult(
					localNodeSelector(node1),
					deviceRequestAllocationResult(req0, driverA, pool1, device2).withConsumedCapacity(&fixedShareID, map[resourceapi.QualifiedName]resource.Quantity{capacity0: one}),
		...
		"consumable-capacity-dedicated-device-with-multiple-consumable-capacity-requests": {
			features: Features{
				ConsumableCapacity: true,
			},
			claimsToAllocate: objects(
