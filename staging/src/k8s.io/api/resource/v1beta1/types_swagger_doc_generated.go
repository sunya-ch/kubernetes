...
var map_CapacityRequirements = map[string]string{
	...
	"requests": "Requests represent individual device resource requests for distinct resources, all of which must be provided by the device.\n\nFor each capacity with a defined SharingPolicy, the policy is applied: - If no request amount is provided, the Default value is applied. - If the capacity's SharingPolicy defines a ValidRange, the value is used as-is or rounded up\n  to the nearest valid amount according to the policy.\n- If the capacity's SharingPolicy defines ValidValues,\n  the value must be in the defined list to be selectable and used as-is.\nThis applied value is used during scheduling to determine how much capacity to consume by the request.\n\nFor each capacity without SharingPolicy, this value is used as an additional filtering condition against the available capacity on the device. This is semantically equivalent to a CEL selector with `device.capacity[<domain>].<name>.compareTo(quantity(<request quantity>)) >= 0`. For example, device.capacity['test-driver.cdi.k8s.io'].counters.compareTo(quantity('2')) >= 0.",
}
...
var map_CapacitySharingPolicyRange = map[string]string{
	...
	"min":  "Min specifies the minimum capacity allowed for a consumption request.\n\nMin must be less than or equal to the capacity value. If Default is defined, it must be more than or equal to the minimum.",
	"max":  "Max defines the upper limit for capacity that can be requested.\n\nMax must be less than or equal to the capacity value. Min and Default (if set) must be less than or equal to the maximum.",
	"step": "Step defines the step size between valid capacity amounts within the range.\n\nMax and Default (if set) must be a multiple of Step. Min + Step must be less than or equal to the capacity value.",
}
...
var map_DeviceCapacity = map[string]string{
	...
	"value":         "Value defines how much of a certain capacity that device has.\n\nThis field reflects the fixed total capacity and does not change. If the capacity is consumable (i.e., a SharingPolicy is defined), the consumed amount is tracked separately and does not affect this value",
	"sharingPolicy": "SharingPolicy defines how this DeviceCapacity must be consumed when the device is allowed to be shared by multiple allocations.\n\nThe Device must have AllowMultipleAllocations set to true.\n\nIf this field is unset, capacity sharing is unconstrained: all ResourceClaims or requests share the same capacity pool.",
}
...
var map_DeviceRequestAllocationResult = map[string]string{
	...
	"consumedCapacities": "ConsumedCapacities tracks the amount of capacity consumed per device as part of the claim request. The consumed amount may differ from the requested amount: it is rounded up to the nearest valid value based on the device’s sharing policy if applicable and must not be less than the requested amount.\n\nThe total consumed capacity for each device must not exceed the DeviceCapacity's Value.\n\nThis field references only consumable capacities of a device and is empty when there are none.",
}
