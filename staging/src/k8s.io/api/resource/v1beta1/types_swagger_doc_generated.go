...
var map_DeviceRequestAllocationResult = map[string]string{
	...
	"consumedCapacity": "ConsumedCapacity tracks the amount of capacity consumed per device as part of the claim request. The consumed amount may differ from the requested amount: it is rounded up to the nearest valid value based on the device’s sharing policy if applicable and must not be less than the requested amount.\n\nThe total consumed capacity for each device must not exceed the DeviceCapacity's Value.\n\nThis field is populated only for devices that support multiple allocations. It references only DeviceCapacity entries that have a specified sharingPolicy, and is empty if no such entries exist.",
}
