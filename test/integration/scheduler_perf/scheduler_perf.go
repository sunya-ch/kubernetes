...
func setupTestCase(t testing.TB, tc *testCase, featureGates map[featuregate.Feature]bool, outOfTreePluginRegistry frameworkruntime.Registry) (informers.SharedInformerFactory, ktesting.TContext) {
	...
	// Reset CEL compiler
	emulationVersion := utilfeature.DefaultFeatureGate.(featuregate.MutableVersionedFeatureGate).EmulationVersion()
	t.Logf("Resetting compiler: %s", emulationVersion, featureGates)
	dracel.ResetCompiler(emulationVersion)
